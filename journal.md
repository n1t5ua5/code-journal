...
Checked on all the e-mails in my scheduled folder inside of Weebly. I've been using them as a host for websites for years now because they also provide e-mail marketing. This newsletter is set to go out tomorrow so I figured it couldn't hurt to look it over them one more time; in a little over a year from now I'll have finished this digital campaign I strategized. It's been nothing short of a success so far: 99% delivery rate, over 40% open rate, over 13% click rate + all subscribers get a free packet of organic seeds annually. It's mostly a data driven newsletter, where we publish different polling on a variety of subjects like working remotely & bioengineered plants. After, I started where I left off on getting this form to work correctly in a different project hosted on Pair Networks, but so far only a few e-mails have managed to arrive in the proper gmail account's inbox, but apparently that might not be the best way to go about this. Maybe I should register a new e-mail address with Pair. Then run the following command in the directory where my website's files reside: cp /usr/www/cgi-bin/recaptcha_formmail.pl, next I will have to perform a few more tasks after creating a new account with Google reCAPTCHA. This is all according to the documentation provided by the host which I linked above.
